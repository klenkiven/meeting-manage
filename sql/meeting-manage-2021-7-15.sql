...
-- 正在导出表  meeting_manage.m_user 的数据：~0 rows (大约)
...
CREATE TABLE IF NOT EXISTS `sys_menu` (
  ...
  PRIMARY KEY (`menu_id`)
) ENGINE=InnoDB AUTO_INCREMENT=86 DEFAULT CHARSET=utf8mb4 COMMENT='菜单管理';

-- 正在导出表  meeting_manage.sys_menu 的数据：~31 rows (大约)
...
INSERT INTO `sys_menu` (`menu_id`, `parent_id`, `name`, `url`, `perms`, `type`, `icon`, `order_num`) VALUES
	...
	(69, 0, '员工信息管理', 'meeting/member', '', 1, 'admin', 2),
	...
	(77, 69, '查看', '', 'meeting:user:list,meeting:user:info', 2, '', 0),
	(78, 69, '新增', '', 'meeting:user:save', 2, '', 0),
	(79, 69, '修改', '', 'meeting:user:update', 2, '', 0),
	(80, 69, '删除', '', 'meeting:user:delete', 2, '', 0),
...
-- 正在导出表  meeting_manage.sys_role 的数据：~2 rows (大约)
...
-- 正在导出表  meeting_manage.sys_role_menu 的数据：~32 rows (大约)
...
-- 正在导出表  meeting_manage.sys_user 的数据：~1 rows (大约)
/*!40000 ALTER TABLE `sys_user` DISABLE KEYS */;
INSERT INTO `sys_user` (`user_id`, `username`, `password`, `salt`, `email`, `mobile`, `status`, `create_user_id`, `create_time`) VALUES
	(1, 'admin', '9ec9750e709431dad22365cabc5c625482e574c74adaebba7dd02f1129e4ce1d', 'YzcmCZNvbXocrsz9dm8e', 'root@tyut4113.org', '18835497385', 1, 1, '2021-07-13 11:11:11');
...
-- 正在导出表  meeting_manage.sys_user_role 的数据：~5 rows (大约)
...
-- 正在导出表  meeting_manage.sys_user_token 的数据：~2 rows (大约)
/*!40000 ALTER TABLE `sys_user_token` DISABLE KEYS */;
INSERT INTO `sys_user_token` (`user_id`, `token`, `expire_time`, `update_time`) VALUES
	(1, '3b64acfd2158e71d6ad9afa136a99eca', '2021-07-16 04:16:19', '2021-07-15 16:16:19'),
	(2, 'c2df3946ae4945836c059821cdb268cb', '2021-07-15 08:46:51', '2021-07-14 20:46:51');
